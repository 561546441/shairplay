...
raop_rtp_parse_remote(raop_rtp_mirror_t *raop_rtp_mirror, const unsigned char *remote, int remotelen)
{
    ...
    if (remotelen == 4) {
        ...
        sprintf(current, "%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x", 
    ...
    logger_log(raop_rtp_mirror->logger, LOGGER_DEBUG, "netutils_parse_address ret = %d", ret);
    if (ret < 0) {
        return -1;
    }
...
raop_rtp_mirror_thread(void *arg)
{
    ...
    logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "in raop_rtp_mirror_thread");
    ...
    while (1) {
        ...
        if (ret == 0) {
            /* Timeout happened */
            // logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "in raop_rtp_mirror_thread Timeout");
            continue;
        } else if (ret == -1) {
            ...
            if ((packet[0] == 80 && packet[1] == 79 && packet[2] == 83 && packet[3] == 84) || (packet[0] == 71 && packet[1] == 69 && packet[2] == 84)) {
                ...
                if (payloadtype == 0) {
                    ...
                    if (h264.lengthofSPS + h264.lengthofPPS < 102400) {
                        ...
                        h264_data.pts = 0;
                        // raop_rtp_mirror->callbacks.video_process(raop_rtp_mirror->callbacks.cls, &h264_data); # TODO hkh
                    }
                      ...
                      unsigned short *mirror_data_lport)
{
    ...
    if (raop_rtp_mirror->running || !raop_rtp_mirror->joined) {
        logger_log(raop_rtp_mirror->logger, LOGGER_INFO, "raop_rtp_start_mirror failed");
        MUTEX_UNLOCK(raop_rtp_mirror->run_mutex);
        return;
    }
